...
export function TryPanel({ transformation }: { transformation: Transformation }) {
  ...
  // Per-transformation model (upstream #978) wins; then the configured defaults.
  const defaultModelId = transformation.model_id || defaults?.default_transformation_model || defaults?.default_chat_model || ''
